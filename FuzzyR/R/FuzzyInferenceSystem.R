...
evalfis <- function(input_stack, fis, time=1, point_n=101, draw=FALSE) {
    ...
    if (first ||
        ...
        # BEGIN, when AGG_METHOD is 'max', the following line will lead to wrong results, hence it is commented out. 13 Mar 2020, Chao
    ...
    for (k in 1 : data_n) {
        ...
        # BEGIN, bug fix (when first input is not used), 13 Mar 2020, Chao
        ind[ind == 0] <- 1
        # END, 13 Mar 2020, Chao
